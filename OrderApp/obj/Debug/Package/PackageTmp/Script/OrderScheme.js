...
// end region get docuemt 

// region loading first product item 
...
// end region loading first product item 


// region get product list first time open page 
function GetProcustlist() {
    $.ajax({
        type: "POST",
...
// end region get product list first time open page 

// region change event tabel row 
        ...
        $tblrow.find(".btnfreemodel").on("click", function myfunction() {
            ...
            if ($tblrow.find('.item option:selected').val() == "") {
                ...
                for (var i = 0; i < arrOrderProductDetail.length; i++) {
                    ...
                    {
                        //alert("btnfreemodel");
                        //alert(arrOrderProductDetail[i].Schemetext);
                        fillOrderProductDetails(arrOrderProductDetail[i].PKG, arrOrderProductDetail[i].QTY, arrOrderProductDetail[i].PackingType, arrOrderProductDetail[i].PackingNos, arrOrderProductDetail[i].isscheme, arrOrderProductDetail[i].srno, arrOrderProductDetail[i].arrOrderProductDetailscounterdata, arrOrderProductDetail[i].PckTotalKg, arrOrderProductDetail[i].Schemetext, arrOrderProductDetail[i].ProductPckID, arrOrderProductDetail[i].SchemeId);
                    }
                }
...
// end region change event tabel row 


//  region add row button click event add new row 
$("#addrow").on("click", function ()
{
    ...
    if (isblankcolumn == false)
    {
        ...
        newRow.append(cols);
        $("table.order-list").append(newRow);
        DynamicGetProcustlist("newdata" + counter);
    }
    $tblrows = $("#tblitemScheme tbody tr");
...
//  end region add row button clic event add new row 

//region btndelete add row below
...
// end region btndelete add row below

//region popup page delete button 
...
// end region popup page delete button 


// region add row form shcem array create 
...
// end region add row form shcem array create 

// region select item data from scheme
...
// region select item packing  data from scheme popup page
...
// end region select item packing  data from scheme popup page
...
// Region save data product packing 
$(".SaveOrderProductDetail").on("click", function myfunction() {
    ...
    //cols += '<td class="col-sm-2"><label  id="scheme" class="scheme">' + $("#dropschemeId option:selected").text() + '</label></td>';
    ...
    cols += '<input type="text" id="Schemetextdata" class="Schemetextdata" value=' + $("#dropschemeId option:selected").text() + '>';
    cols += '<input type="text" id="SchemeId" class="SchemeId" value=' + $("#dropschemeId option:selected").val() + '>';
    cols += ' </td>';
    cols += '<td class="col-sm-2"><label  id="NOS" class="NOS">' + $("#drppkg option:selected").attr("UOM") + '</label></td>';
    cols += '<td class="col-sm-2"><label  id="scheme" class="scheme">' + $("#dropschemeId option:selected").text() + '</label></td>';
...
// end Region save data product packing 
...
//#region orderproduct details 
function fillOrderProductDetails(pkg, qty, packingtype, packingnos, isscheme, srno, arrOrderProductDetailscounterdata, PckTotalKg, 
			Schemetextdata, ProductPckId, SchemeId) {
    ...
    cols += '<input type="text" id="SchemeId" class="SchemeId" value=' + SchemeId + '>';
    cols += ' </td>';
    cols += '<td class="col-sm-2"><label  id="NOS" class="NOS">NOS</label></td>';
    cols += '<td class="col-sm-2"><label  id="Schemetextdatavisible" class="Schemetextdatavisible">' + Schemetextdata + '</label></td>';
    var totalkgcount = 0, totalfree = 0;
    ...
    cols += '<td class="col-sm-2"><label  id="overalltotalfreekg" class="overalltotalfreekg">' + parseFloat(totalfree) + '</label></td>';
    if ($("#ContentPlaceHolder1_isview").val() == "1") {
        cols += '<td  class="col-sm-1"> <input type="button" style="display:none"></td>';
    }
    else {
        cols += '<td  class="col-sm-1"> <input type="button" class="ibtnDelProdRows btn btn-danger "  value="Delete"></td>';
...
//#end region orderproduct details 

//  region clear popdata 
function clearpopupdatadtl() {
    ...
    $("#package").val('');
    $("#popuplistfreekg").val('');
    $(".drpscheme option").remove();
}
// end region clear popdata 
//region total  popup page no row total kg
...
// end region total  popu page no row total kg
...
function SaveClose() 
{
    ...
    $("#tblOrderProductDetail tbody tr").each(function ()
    {
        ...
        SingleArrOrderProductDetail.SchemeId = $(this).find("td").find("#SchemeId").val();

        SingleArrOrderProductDetail.overalltotalfreekg = $(this).find("td").find("#overalltotalfreekg").val();
        //alert($(this).find("td").find("#Schemetextdata").val());
        arrOrderProductDetail.push(SingleArrOrderProductDetail);

    });
    $("#tblOrderProductDetail tbody").html('');
}
// region  get order scheme data tablefill in array 
...
// end region  get order scheme data tablefill in array 

// region fill product packing drop down
function getProductPacking(ids) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(jQuery.parseJSON(data.d), function (key, item) {
                $(".drpProductPacking").append($("<option value=" + item.ProductPckID + " PackingNos=" + item.PackingNos + " PackingType=" + item.PackingType + " ProductPck=" + item.ProductPck + " TotalKG=" + item.TotalKG + " IsScheme=" + item.IsScheme + " UOM=" + item.UOM + " >" + item.ProductPck + "</option>"));
            });
        },
        failure: function () {
            alert("Failed!");
...
// end region fill product packing drop down

// region get pakcing wise scheme dropdwon fill 
$("#drppkg").on('change', function myfunction() {
   ...
   var Stateid = $("#ContentPlaceHolder1_hdstate").val();
    ...
    var ProductPckId = ($("#drppkg option:selected").attr("value"));
    $.ajax({
        type: "POST",
        url: "AddDealerOrderScheme.aspx/GetProductPackingScheme",
        data: '{ProductPckId:' + ProductPckId + ', Stateid : ' + Stateid + ' }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            //$(".drpscheme").html($("<option></option>").val('').html('---select---'));
            $.each(jQuery.parseJSON(data.d), function (key, item) {
                $(".drpscheme").append($("<option value=" + item.SchemeId + " >" + item.SchemeName + "</option>"));
            });
        },
        failure: function () {
            alert("Failed!");
...
// end region get pakcing wise scheme dropdwon fill 

// region first time by deafualt call item call 
...
// end region first time by deafualt call item call 

// region add new row time this dropdow fill in product list
function DynamicGetProcustlist(drpname) {
    $.ajax({
        type: "POST",
        url: "AddDealerOrderScheme.aspx/GetProcustlist",
        data: '{name: "abc" }',
...
// end region add new row time this dropdow fill in product list

// region open poup page 
$("#btnfreemodel").on("click", function myfunction() {
    $("#package").text('');
    $("#modelfreeitem").modal("show");
});
// end region open poup page 
        ...
        if (totalfree != $(this).find('td').find('.TotalFreeKg').val()) {
            ...
            for (var i = 0; i < arrOrderProductDetail.length; i++) {
                if (arrOrderProductDetail[i].srno == $("#srnoProd").val()) // delete index
                {
				    ...
				    fillOrderProductDetails(arrOrderProductDetail[i].PKG, arrOrderProductDetail[i].QTY, arrOrderProductDetail[i].PackingType,
					 arrOrderProductDetail[i].PackingNos, arrOrderProductDetail[i].isscheme, arrOrderProductDetail[i].srno, 
					 arrOrderProductDetail[i].arrOrderProductDetailscounterdata, arrOrderProductDetail[i].PckTotalKg,
					  arrOrderProductDetail[i].Schemetext, arrOrderProductDetail[i].ProductPckID, arrOrderProductDetail[i].SchemeId);
    ...
    if (formvalidation() == true) {
        ...
        if (parseInt($("#ContentPlaceHolder1_txtPurchaseKg").val()) > parseInt(pkg)) {
            ...
            $.ajax({
                ...
                data: "{Stateid: " + Stateid + ", data:'" + JSON.stringify(data) + "',productFreedata:'" + JSON.stringify(orderdata) + "',OrderProductDetails:'" + JSON.stringify(arrOrderProductDetail) + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (data) {
...
// #end region save date 

// region validateion addrow order list page free scheme
...
// end region validateion addrow order list page free scheme

// region save datatime forma validation 
...
// end region save datatime forma validation 

//region Get Data From free prodcut  id
...
// end region Get Data From free prodcut  id

// region get order prodcut details funcation 
        ...
        success: function (data) {
            $.each(jQuery.parseJSON(data.d), function (key, item) {
                ...
                SingleArrOrderProductDetail.overalltotalfreekg = item.TotalKg;
				 SingleArrOrderProductDetail.SchemeId = item.SchemeId;
                SingleArrOrderProductDetail.ProductCode = item.ProductCode;
                arrOrderProductDetail.push(SingleArrOrderProductDetail);
...
// end region view modelorder edit
...
// end Dynamic Table add or remove
