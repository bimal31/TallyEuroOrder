...
$(document).ready(function () {
    if ($("#ContentPlaceHolder1_hdOrdertype").val().toLowerCase() != "order") {
...
function GetBrandCategory() {
    $.ajax({
        type: "POST",
        url: "FreeOrder.aspx/GetProcustlist",
        data: '{name: "abc" }',
...
function GetSelectOrderProductDetails(orderid) {
    $.ajax({
        type: "POST",
        url: "FreeOrder.aspx/GetOrderProductDetails",
    ...
    $tblrows.each(function (index) {
        ...
        $tblrow.find('.txtQty').on('change', function (event) {
            if ($tblrow.find('.txtQty').val() != '') {
                if ($tblrow.find("#drpProductPacking option:selected").attr("packingtype") != "gm") {
                    Purchase = (parseFloat($tblrow.find('.txtQty').val()) * parseFloat($tblrow.find("#drpProductPacking option:selected").attr("productpck")));
                } else {
                    Purchase = (parseFloat($tblrow.find('.txtQty').val()) * parseFloat($tblrow.find("#drpProductPacking option:selected").attr("productpck"))) / 1000;
                }
                ...
                $("#ContentPlaceHolder1_txtFreeKg").val(freecount);
            }
...
function getordertabledata() {
    ...
    $("table.order-list tbody tr").each(function () {
        ...
        var ProductCode = $(this).find("td").eq(4).find("select option:selected").attr("ProductCode");
...
function selectionitem(thisval, drppos) {
    ...
    $("#ContentPlaceHolder1_grdProductFree tr").each(function () {
       
        
        var rowtype = $(this).attr("class");
        //var headerProductId = $(this).find("th:eq(0)").html();
        if (rowtype == 'rows')
        {
           // alert($(this).attr("productid"));
            if ($(this).attr("productid") == selected)
            {
                isexist = true;
            }
        }
    ...
    if (isexist == true)
    {
        ...
        alert("please select existing product.");
    }
}
//endregion 
...
function getProductPacking(ids, drppos) {
    $.ajax({
        type: "POST",
        url: "FreeOrder.aspx/GetProductPacking",
        data: '{id:' + ids + ' }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
...
function selectionitemQty(thisval, drppos) {
    ...
    if (ids != '') {
        ...
        $.ajax({
            type: "POST",
            url: "FreeOrder.aspx/GetProductPackingScheme",
            ...
            success: function (data) {
                $.each(jQuery.parseJSON(data.d), function (key, item) {
                  //$(".ProductPackingscheme" + drppos + "").append($("<option value=" + item.SchemeId + " >" + item.schemeProductCode + "</option>"));
                    $(".ProductPackingscheme" + drppos + "").append($("<option value=" + item.SchemeId + " data-productcode = " + item.schemeProductCode + ">" + item.SchemeName + "</option>"));
                });
                purchageonchange();
            },
            failure: function () {
...
$("#btnSubmitOrder").on("click", function myfunction() {
    ...
    if (formvalidation() == true)
    {
        ...
        var ReturnId = 0;
        var flg = "0";
        var Totalqty = $("#ContentPlaceHolder1_txtFreeKg").val();
       ...
       // return;
        var data = {};
        data.OrderType = "With Bill Free Scheme";
        ...
        //data.FreeSchemeFrom = $("#ContentPlaceHolder1_txtFromScheme").val();
        //data.FreeSchemeTO = $("#ContentPlaceHolder1_txtToScheme").val();
        //data.TotalKgGm = $("#ContentPlaceHolder1_txtFreetotalkg").val();
        data.ParentOrderId = $("#ContentPlaceHolder1_hdEditOrderId").val();
        data.TotalKgGm = $("#ContentPlaceHolder1_txtFreeKg").val();

        var editid = $("#ContentPlaceHolder1_hdFreeOrderId").val();
        ...
        $.ajax({
            type: "POST",
            url: "FreeOrder.aspx/SaveData",
            data: "{Stateid: " + Stateid + ", data:'" + JSON.stringify(data) + "',OrderProductDetails:'" + JSON.stringify(orderdata) + "',OrderFreeProductDetails:'" + JSON.stringify(arrOrderProductDetail) + "'}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (data) {
...
function callgirdcount()
{
    var MatchData = true;

    var FinaltotalKg = 0;
    var PurchaseTotalkgLoop = 0;
    var ProductName = "";
    var prodcuttotalkg = 0;
    $("#ContentPlaceHolder1_grdProductFree tr").each(function ()
    {
        ...
        if (rowtype == 'rows')
        {
            ...
            var Purchasekg = $(this).find("td:eq(4) span").html();
            PurchaseTotalkgLoop = parseFloat(PurchaseTotalkgLoop) + parseFloat(Purchasekg);
            ProductName = $(this).find("td:eq(0)").html();
             $("#tblitemScheme tbody tr").each(function ()
            {
               
              //  alert(prodcuttotalkg);
                if ($("#tempdrpid").val() == prodcutid)
                {
                   // alert(parseFloat($(this).find('.ItemTotalKg').val()));
                    prodcuttotalkg = parseFloat(prodcuttotalkg) + parseFloat($(this).find('.ItemTotalKg').val());
                   // alert(prodcuttotalkg);
                }
            });
            FinaltotalKg = FinaltotalKg + prodcuttotalkg;
    ...
    if (FinaltotalKg != PurchaseTotalkgLoop) {
        //alert("Total Kg not match this prodcut " + ProductName);
        alert("Total Kg not match Free Kg.");
        MatchData = false;
        return MatchData;
    }
    else {
...
function gridgetdata()
{
    ...
    $("#ContentPlaceHolder1_grdProductFree tr").each(function ()
    {
        ...
        if (rowtype == 'rows')
        {
            ...
            var ProductName = $(this).find("td:eq(0) span").html();
            var Totalkg = $(this).find("td:eq(1) span").html();
            var Fromscheme = $(this).find("td:eq(2)").html();
            var Toscheme = $(this).find("td:eq(3)").html();
            var FreeSchemeTotalkg = $(this).find("td:eq(4) span").html();
            
            FreeCount = FreeCount + 1;
            makearraytableProductdetails(ProductID, ProductName, Fromscheme, Toscheme, Totalkg, FreeSchemeTotalkg, FreeCount);
...
function makearraytableProductdetails(ProductID, ProductName, Fromscheme, Toscheme, Totalkg, FreeSchemeTotalkg, FreeCount) {
   ...
   // data.item = item;
    ...
    data.Totalkg = Totalkg;
    data.FreeSchemeTotalkg = FreeSchemeTotalkg;
    data.OrderSrNo = FreeCount;
    arrOrderProductDetail.push(data);
}
...
function formvalidation()
{
    ...
    if (callgirdcount() == true)
    {
        ...
        //else if (parseFloat($("#ContentPlaceHolder1_txtFreeKg").val()) != parseFloat($("#ContentPlaceHolder1_txtFreetotalkg").val())) {
        //    alert("Total Free kg not match");
        //    return isvalid;
        //}
        else {
            isvalid = true;
